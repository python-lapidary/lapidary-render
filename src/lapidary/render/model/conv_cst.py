...
def type_hint_to_attribute(
    ...
    if isinstance(typ, python.NoneTypeHint):
        return mk_name('None')

    if typ.module == 'builtins':
        origin = cst.Name(typ.name)
    else:
        parts = typ.module.split('.')
        origin = mk_name(*parts, typ.name)

    if isinstance(typ, python.GenericTypeHint):
        return mk_parametrized_type(origin, [type_hint_to_attribute(arg, indent + 1) for arg in typ.args], indent + 1)
    elif isinstance(typ, python.TypeHint):
        return origin
    else:
        raise TypeError(typ, type(typ))
...
def mk_class_field_stmt(field: python.Field) -> cst.SimpleStatementLine:
    args = [
        cst.Arg(
            keyword=cst.Name(key),
            value=mk_raw_str_literal(value) if key == 'pattern' else mk_literal(value),
        )
        for key, value in field.annotation.field_props.items()
    ]
    metadata = []
    if args:
        metadata.append(
    ...
    logger.info('indent for %s: %d', field.name, 0)
    return cst.SimpleStatementLine(
        body=[
            cst.AnnAssign(
                target=cst.Name(field.name),
                annotation=cst.Annotation(mk_annotation(field.annotation.type, metadata, 0)),
...
def mk_annotation(
    ...
    if metadata:
        ...
        return type_hint_to_attribute(typ, indent)
...
def mk_operation_params(operation: python.OperationFunction) -> Iterator[cst.Param]:
    for idx, param in enumerate(operation.params):
        ...
        in_ = mk_name('lapidary', 'runtime', param.annotation)
        ...
        annotation = cst.Annotation(mk_annotation(param.type, [mk_call(in_, param_args, 3) if param_args else in_], 1))
...
def mk_response(response: python.Response) -> cst.Call:
    ...
    if response.headers_type is not python.NONE:
...
def mk_metadata_field(model: python.MetaField) -> cst.SimpleStatementLine:
    param_args = []
    if model.alias:
    ...
    in_ = mk_name('lapidary', 'runtime', model.annotation)

    annotation = cst.Annotation(mk_annotation(model.type, [mk_call(in_, param_args, 2) if param_args else in_], 0))
    return cst.SimpleStatementLine(
        body=[
            cst.AnnAssign(
