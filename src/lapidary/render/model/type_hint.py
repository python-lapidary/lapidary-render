...
def get_type_hint(schema: openapi.Schema, module: python.ModulePath, name: str, required: bool,
                  resolver: ResolverFunc) -> python.type_hint.TypeHint:
    typ = _get_type_hint(schema, module, name, resolver)

    if schema.nullable:
        typ = typ.union_with(python.type_hint.BuiltinTypeHint.from_str('None'))
    if not required:
        typ = typ.union_with(python.type_hint.TypeHint.from_type(Absent))

    return typ


def _get_one_of_type_hint(schema: openapi.Schema, module: python.ModulePath, name: str, resolve: ResolverFunc) -> python.type_hint.TypeHint:
    args = []
    ...
    return python.type_hint.GenericTypeHint(
        module='typing',
        name='Union',
        args=tuple(args),
    )
...
def _get_composite_type_hint(
        component_schemas: list[typing.Union[openapi.Schema, openapi.Reference]], module: python.ModulePath, name: str, resolve: ResolverFunc
...
def _get_type_hint(schema: openapi.Schema, module: python.ModulePath, name: str, resolver: ResolverFunc) -> python.type_hint.TypeHint:
    class_name = name.replace(' ', '_')
    if schema.enum:
        return python.type_hint.TypeHint(module=module.str(), name=class_name)
    elif schema.type == openapi.Type.string:
        return python.type_hint.TypeHint.from_type(STRING_FORMATS.get(schema.format, str))
    elif schema.type in PRIMITIVE_TYPES:
        return python.type_hint.BuiltinTypeHint.from_str(PRIMITIVE_TYPES[schema.type].__name__)
        ...
        return python.type_hint.TypeHint.from_str('typing:Any')
    else:
        raise NotImplementedError


def _get_type_hint_object(schema: openapi.Schema, module: python.ModulePath, name: str) -> python.type_hint.TypeHint:
    if schema.properties or schema.allOf:
        return python.type_hint.TypeHint(module=module.str(), name=name)
    else:
        return python.type_hint.TypeHint(module=module.str(), name=name)
...
def resolve_type_hint(typ: typing.Union[openapi.Schema, openapi.Reference], module: python.ModulePath, name: str,
                      resolver: ResolverFunc) -> python.type_hint.TypeHint:
